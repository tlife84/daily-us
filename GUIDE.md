...
캡처는 사이트의 모바일 레이아웃(본문 폭 430px)으로 찍습니다. 데스크톱 레이아웃은 본문 폭이 660px라 같은 글이 폰에서 그만큼 작게 보입니다. 텔레그램이 사진의 긴 변을 2560px로 줄이므로, 블록 경계를 따라 그보다 짧게 잘라야 축소 없이 도착합니다. 이 과정에서 맨 위 표지 이미지와 화면에 고정된 배너, 헤더, 버튼을 함께 제거합니다.
