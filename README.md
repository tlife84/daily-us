...
US Insight 피드에서 특정 제목의 게시글을 확인하고, 본문·오디오·PDF를 텔레그램으로 보내는 자동화 도구입니다. 화요일 정규수업 영상은 Google Drive에 업로드하고 봇으로 링크를 보냅니다.

자세한 운영 방법은 [GUIDE.md](GUIDE.md)를 참고하세요.

정규수업은 화요일 **20:05~22:00, 5분 간격(한국 시간)**으로 확인합니다. 최초 사용 전 [Drive 인증 및 예약 등록 안내](GUIDE.md#10-정규수업-동영상)를 따라 Google OAuth 인증을 완료하고 운영체제 예약 작업을 갱신하세요.
...
브라우저에서 네이버 로그인과 동의·회원 연결을 완료하면, 인증 성공과 피드 진입을 확인해 세션을 자동으로 저장합니다. 터미널에서 Enter를 누를 필요가 없습니다. `로그인 세션이 저장되었습니다` 안내와 함께 브라우저가 자동으로 닫힐 때까지 기다리세요.

같은 인증 파일을 사용하는 작업은 순서대로 실행하며, 피드 조회·본문 캡처·로그인 확인 중 갱신된 인증 상태도 저장합니다. 다만 서버에서 갱신 토큰을 무효화했거나, 실행하지 않은 기간에 토큰이 만료되면 다시 로그인해야 합니다.
